...
class HalmaGameService(rpyc.Service):
    ...
    jogadores_conectados = {}  # dict mapping conexao -> player_id
    ...
    def on_connect(self, conn):
        ...
        with HalmaGameService.lock:
            player_id = len(HalmaGameService.jogadores_conectados) + 1
            HalmaGameService.jogadores_conectados[conn] = player_id
    ...
    def on_disconnect(self, conn):
        ...
        with HalmaGameService.lock:
            if conn in HalmaGameService.jogadores_conectados:
                del HalmaGameService.jogadores_conectados[conn]
    
    def exposed_registrar_jogador(self):
        # Registra um novo jogador e retorna seu ID e status de host
        # Obtem a conexao atual do RPyC
        conn_atual = self._conn
        with HalmaGameService.lock:
            if conn_atual in HalmaGameService.jogadores_conectados:
                player_id = HalmaGameService.jogadores_conectados[conn_atual]
                is_host = (player_id == 1)
